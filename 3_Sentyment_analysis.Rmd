...
# Inspect the Syuzhet dictionary
...
## First create the filtered dataframes

```{r, echo=TRUE}
# Create filtered dataframes
MELONI <- dataset %>% filter(nome  %like% "MELONI")
CONTE <- dataset %>% filter(nome  %like% "CONTE")
RENZI <- dataset %>% filter(nome  %like% "RENZI")
SALVINI <- dataset %>% filter(nome  %like% "SALVINI")
LETTA <- dataset %>% filter(nome  %like% "LETTA")
BERLUSCONI <- dataset %>% filter(nome  %like% "BERLUSCONI")
SPERANZA <- dataset %>% filter(nome  %like% "SPERANZA")
```

## Then create nrc objects
...
# 1) Giorgia Meloni - TRUST - GOVERNO
### Plot the percentage of the emotion
...
### Plot the wordcloud of emotions
