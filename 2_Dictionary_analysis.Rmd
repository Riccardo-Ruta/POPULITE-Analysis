...
This plot is coherent with the previous one and show us that people is thr Component that score better. 
...
```{r}
x <- dict_4_tstat_party$frequency

d <- density(dict_4_tstat_party$frequency) 
d$y <- d$y * length(dict_4_tstat_party$frequency)
plot(d)
```
...
```{r}
x <- dict_4_tstat_nome$frequency

d <- density(dict_4_tstat_nome$frequency) 
d$y <- d$y * length(dict_4_tstat_nome$frequency)
plot(d)
```
...
comparison_time <- plot(dat_smooth4$x, dat_smooth4$y, type = "l", ylab = "Populism level", xlab = "Time", col = "green", ylim = c(0,5)) + abline(h = 0, lty = 2) +
  lines(dat_smooth3$x, dat_smooth3$y, type = "l", ylab = "Populism level", xlab = "Time", col = "blue") + abline(h = 0, lty = 2) +
  lines(dat_smooth2$x, dat_smooth2$y, type = "l", ylab = "Populism level", xlab = "Time", col = "red") +abline(h = 0, lty = 2)
legend("topleft", legend = c("Roodujin_pauwels","Grundl_italian","Decadri_Boussalis"), fill = c("red", "blue", "green")) 
```
