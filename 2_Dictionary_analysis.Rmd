...
The code is only shown for the "PEOPLE" component but is identical for the others
...
plot_people <- ggplot(data = data_quarter_people, aes(x = Group.1, y = perc))+
  ...
  scale_x_continuous("Quarters", labels = as.character(data_quarter_people$Group.1),
                     breaks = data_quarter_people$Group.1)+
...
p <- ggplot() +
  # plot people
  geom_line(data = data_quarter_people,
            aes(x = Group.1, y = perc, color = "people"), size = 2) +
  # plot common will
  geom_line(data = data_quarter_common,
            aes(x = Group.1, y = perc, color = "common will"), size = 2) +
  # plot elite
  geom_line(data = data_quarter_elite,
            aes(x = Group.1, y = perc, color = "elite"), size = 2) +
...
plot_general <- ggplot(data = data_quarter_general, aes(x = Group.1, y = perc))+
  ...
  scale_x_continuous("Quarters", labels = as.character(data_quarter_general$Group.1),
                     breaks = data_quarter_general$Group.1)+
  ylab("Percentage of populist words")+
...
The code is only shown for the main "POPULISM"  indicator 
but is identical for the single components
...
kable(data_party_people %>% select(Group.1, perc)%>% arrange(desc(perc)), col.names = c("Party","Perc"),
      caption = "People")%>%
  kable_styling(latex_options = "HOLD_position")
...
kable(data_party_common %>% select(Group.1, perc)%>% arrange(desc(perc)), col.names = c("Party", "Perc"),
      caption = "Common will")%>%
  kable_styling(latex_options = "HOLD_position")
...
kable(data_party_elite %>% select(Group.1, perc)%>% arrange(desc(perc)), col.names = c("Party","Perc"),
      caption = "Elite")%>%
  kable_styling(latex_options = "HOLD_position")
...
ggplot(left_party, aes(x=quarter, y=populism, color=party_id)) +
  geom_line(size=1.5)+
  scale_x_continuous("Quarters", labels = as.character(left_party$quarter),
                     breaks = left_party$quarter)+
  ylab("Percentage of populist words")+
...
ggplot(right_party, aes(x=quarter, y=populism, color=party_id)) +
  geom_line(size=1.5)+
  scale_x_continuous("Quarters", labels = as.character(right_party$quarter),
                     breaks = right_party$quarter)+
  ylab("Percentage of populist words")+
...
# By quarter
dfm_dict2  <- dfm_lookup(dfm_weigh_p_quart,
                         dictionary = Rooduijn_Pauwels_Italian)
...
# Dictionary analysis with Rooduijn_Pauwels_Italian
# By quarter
dfm_dict3  <- dfm_lookup(dfm_weigh_p_quart,
                         dictionary = Grundl_Italian_adapted)
