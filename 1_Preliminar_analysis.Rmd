...
n_parl_party <- dataset %>% select(party_id, nome) %>% group_by(party_id) %>% unique() %>% count()
kable(n_parl_party)

# Gender composition
n_gender <- dataset %>% select(genere, nome) %>% group_by(genere) %>% unique() %>% count()
kable(n_gender)
...
if_else(party_id == "LEGA", "@matteosalvinimi",                                   if_else(party_id == "IV", "@matteorenzi",
